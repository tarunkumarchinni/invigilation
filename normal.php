...
// echo msg;
  ...
  mysql_connect("localhost","root","");
   ...
   echo "<html><link rel='stylesheet' type='text/css' href='login.css'><form class='form1' action='admin3.php' method='POST'><table border='1'><tr bgcolor='red'><th>faculty id</th><th>first name</th><th>last name</th><th>email</th><th>phone no</th><th>allocate</th></tr>";

  
        $re=mysql_query("select * from ptimetable where day='$m'")or die("failed to query database".mysql_error());	   
			...
			if($row['9:00']==NULL){
	            ...
	            if($r['faculty_id']==$s1){
					$tar=mysql_query("select * from invigilation_table where faculty_id='$s1' and examdate='$date' ")or die("failed to query database".mysql_error());
					 ...
					 if($r1==0){
				        echo "<tr><td>{$r['faculty_id']}</td><td>{$r['fname']}</td><td>{$r['lname']}</td><td>{$r['email']}</td><td>{$r['phone']}</td><td><a href='admin3.php?stu=".$r['faculty_id']."&dat=".$date."&tim=".$time."'>request</a></tr>";
	                 }
					 else{}
